...
func (p *Peer) register(reg prometheus.Registerer, name string) {
	peerInfo := prometheus.NewGauge(
		prometheus.GaugeOpts{
			Name:        "cluster_cluster_peer_info",
	...
	clusterFailedPeers := prometheus.NewGaugeFunc(prometheus.GaugeOpts{
		Name: "cluster_cluster_failed_peers",
	...
	p.failedReconnectionsCounter = prometheus.NewCounter(prometheus.CounterOpts{
		Name: "cluster_cluster_reconnections_failed_total",
	...
	p.reconnectionsCounter = prometheus.NewCounter(prometheus.CounterOpts{
		Name: "cluster_cluster_reconnections_total",
	...
	p.failedRefreshCounter = prometheus.NewCounter(prometheus.CounterOpts{
		Name: "cluster_cluster_refresh_join_failed_total",
	...
	p.refreshCounter = prometheus.NewCounter(prometheus.CounterOpts{
		Name: "cluster_cluster_refresh_join_total",
	...
	p.peerJoinCounter = prometheus.NewCounter(prometheus.CounterOpts{
		Name: "cluster_cluster_peers_joined_total",
...
func (p *Peer) SendReliable(n *memberlist.Node, msg []byte) {
	err := p.mlist.SendReliable(n, msg)
	if nil != err {
		level.Error(p.logger).Log("send msg to %s failed,err:%s", n.Name, err.Error())
	}
}

func (p *Peer) SendBestEffort(msg []byte) {
	for _, pr := range p.peers {
		if pr.Name != p.Self().Name {
			err := p.mlist.SendBestEffort(pr.Node, msg)
			if nil != err {
