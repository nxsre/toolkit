...
func newDelegate(l log.Logger, reg prometheus.Registerer, p *Peer, retransmit int) *delegate {
	...
	healthScore := prometheus.NewGaugeFunc(prometheus.GaugeOpts{
		Name: "cluster_cluster_health_score",
	...
	messagesQueued := prometheus.NewGaugeFunc(prometheus.GaugeOpts{
		Name: "cluster_cluster_messages_queued",
...
func (d *delegate) NotifyMsg(b []byte) {
	if d.Peer.notifyMsg != nil {
		// NotifyMsg 方法不能被阻塞，后台执行 notifyMsg
		go func() {
			if err := d.Peer.notifyMsg(b); err != nil {
				level.Warn(d.logger).Log("call notifyMsg falied:", err)
			}
		}()
	}
