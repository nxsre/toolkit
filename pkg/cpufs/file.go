...
func (n *Node) Read(ctx context.Context, dest []byte, off int64) (fuse.ReadResult, syscall.Errno) {
	...
	logrus.WithField("path", n.path).Debug("Node Read", off)

	n.content = []byte("11111111111")
...
func (n *Node) Create(ctx context.Context, name string, flags uint32, mode uint32, out *fuse.EntryOut) (*fs.Inode, fs.FileHandle, uint32, syscall.Errno) {
	...
	return n.NewInode(ctx, &child, fs.StableAttr{Mode: child.getMode(true), Ino: n.inodeHash(child.path)}), nil, 0, err
}
